...
class MainView(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        ttk.Label(form, text="Data zakupu").grid(row=0, column=4, sticky="w")
        self.date_cb = DateEntry(form, date_pattern="yyyy-mm-dd", state="readonly", width=11, firstweekday="monday", showweeknumbers=False)
        self.date_cb.grid(row=0, column=5, padx=5)
        self.date_cb.set_date(date.today())
        ...
        self.tree.column("purchase_date", width=100, anchor="center")
    ...
    def refresh(self):
        ...
        for r in rows:
            self.tree.insert(
                "", "end", iid=str(r["id"]),
                values = (r["id"], r["name"], r.get("category", "") or "", r.get("purchase_date", "") or "", r.get("serial_number", "") or "")
            )

    def on_add(self):
        name = self.name_var.get().strip()
        category = self.category_var.get().strip()
        purchase_date = self.date_cb.get_date().strftime("%Y-%m-%d")
        ...
        try:
            self.db.add_item(name=name, category=category, purchase_date=purchase_date, serial_number=serial_number)
        except Exception as e:
